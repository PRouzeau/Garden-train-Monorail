...
the batch generator spreadsheet and regenerate the batch files. 
...
This need to activate macros and may need to slect medium security level to operate the macros.
...
While for me this was among my first prints, I think it is preferable to have some
experience in 3D printing for such project. 

Wheels shall be well printed and shall run straight. (from the first protoypes,
wheels were modified to have a monobloc shaft support).
Use calibrated diameter shaft for wheels (3mm).
...
so the print quality of shown parts is not very good. I made many paint layers to hide the
print faults. The real model looks better than the photos as flash underline the defaults.
